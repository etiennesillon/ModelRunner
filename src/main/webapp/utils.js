...
function reloadPost(endpoint, contextid, action, typeid, itemid, attrid) {
   sendReloadPost(endpoint, contextid, action, typeid, itemid, attrid, postReloadContent);
}

function sendReloadPost(endpoint, contextid, action, typeid, itemid, attrid, callback) {

   var updateForm; 
   
   if(action == 24) {  // search (defined in IDCWebAppController.SEARCH)
		updateForm = document.forms.SearchForm;
   } else {
		updateForm = document.forms.UpdateItemDetails;
	} 

   updateForm.action=endpoint;

   addHiddenField(updateForm, "contextid", contextid);
...
function getServerQuery(action, editor, params) {
   ...
   var ret = "ModelEditor?action=" + action + "&editor=" + editor;
