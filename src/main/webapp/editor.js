...
function showUpdateForm(id, x, y) {
   ...
   if(id == -1) {
      ...
      var obj = objects[id];
      var type = obj.type;
      form = new Form(obj.type, obj.item, SHAPE_NAMES[editor][type], "hidePopupPanel(); processFormUpdate(" + id + ");", getFields(type));      showPopupPanel(form.getHTML(), x, y);
   }
...
function drawShapeLink(shape1, shape2) {
   ...
   var x1 = shape1.xm - canvasLeft - boxLeft;
   var y1 = shape1.ym - canvasTop - boxTop;
   var x2 = shape2.xm - canvasLeft - boxLeft;
   var y2 = shape2.ym - canvasTop - boxTop;
...
function processNLUCommand(command) {
   ...
   for (var nWord = 0; nWord < words.length; nWord++) {
      ...
      if(word == 'to' || word == '2'  || word == 'two') {
         nTo = nWord;
      } else if(word == 'called' || word == 'call'  || word == 'cold' || word == 'gold') { // don't add code because it breaks the zip code attribute!
         nCalled = nWord;
      } else if(word == 'clean') {
         isClean = true;
      } else if(nCalled == -1) {
...
function help() {
	...
	var text = "<div style='background-color:aliceblue;border:1px solid black;overflow:scroll;height: 80vh;width: 80vw'>";
	text += "<button onclick='hidePopupPanel();'>Close</button>";
	text += "<h1>Model Runner Help!</h1><p>Hi, this is a (temporary) basic help page on how to use Model Runner:<p>";
	text += "<h2>Create a new application</h3><p>Click on 'New' and specify a name for your new application or click on 'Speak' and say something like 'please create an application called xxxx'. Click on 'Speak' again to finish the speech recognition and process the utterance.</p>";
	text += "<h2>Add a package</h2><p>A package is a logical grouping of related data types. You need to create a package before you can add a data type.\nRight click anywhere on the blue background, select 'New Package' and specify the package's name. Or simply say 'create a package called xxx'.</p>";
	text += "<h2>Add a data type</h2><p>Right click anywhere in the package, select 'New Type' and specify the type's name. Or you can say something like 'add a type called xxxx'.\nA new type will be created with a Main panel which is where attribute are displayed on the UI. You can add other panels to the data type. The Main panel will already include a Name attribute which is the default way of identifying data objects. You can delete or rename the Name attribute but you will need to specify a Name Formula in the Data Type otherwise the objects will all show as '(no name specified)'.</p>";
	text += "<h2>Add an attribute</h2><p>Right click anywhere in the Panel, select 'New Attribute' and specify the Attribute's name. Or you can say something like 'add an attribute called xxxx'. By default attributes are created as String, which is just a Text Field on the UI. You can select between multiple attribute types using the Type drop down box in the Attribute edit form. Most attribute type should be pretty self explanatory, here's a list of more special ones:</p>";
	text += "<ul><li>Text is just a text box as opposed to a text field</li>";
	text += "<li>Domain refers to a hardcoded list of Domain Values in a Domain. You can create a Domain by  right clicking anywhere in a Package and select 'New Domain'. You can add Domain Values by right clicking anywhere in the Domain and select 'New Value'.</li>";
	text += "<li>Ref is a Reference Attribute. You will need to specify a target Data Type. RefBox is a special version of Ref which will display the name of objects in the target dat type in a drop down box. RefTree is similar to RefBox but it will look for a chain of NameSpace attributes linking to the target data type and offer a drop down box to select the objects parents first.</li>";
	text += "<li>Aggregation allows you to select multiple objects of the target type</li>";
	text += "<li>Composition allows you to create multiple object of the target type within the namespace of the current object</li>";
	text += "<li>Extension will add all attributes of the target type to the current type</li>";
	text += "<li>Back Reference: you will need to specify a target attribute and the value of the Back Reference attribute will be the list of objects which has the current object as value of the target attribute. It’s hard to describe but just try it :)</li></ul>";
	text += "<h2>Editing objects</h2><p>Double click on an object to edit it or right click and select 'Edit'.</p>";
	text += "<h2>Moving objects</h2><p>Click and hold anywhere inside an object to move it.</p>";
	text += "<h2>Resizing</h2><p>If you hover over an object’s border, the line will become wider and if you click and hold, you will be able to resize it horizontally and vertically</p>";
	text += "<h2>Deploying the model<p></h2><p>You can deploy your model at any stage by just clicking on 'Deploy' or saying something like 'Deploy the Application'. You can also save it in XML format by clicking on 'Download' and you will be able to reload it using 'Upload'. To make changes to a deployed model, click on 'Open'' and select the model's name</p>"; 
	text += "<button onclick='hidePopupPanel();'>Close</button>";
	text += "</div>"; 

	showPopupPanel(text, 100, 100);
