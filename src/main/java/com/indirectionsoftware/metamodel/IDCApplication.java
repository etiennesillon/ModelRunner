...
public class IDCApplication  extends IDCModelData {
	...
	public void addPackages(List<IDCPackage> packages, boolean isSystem, IDCDatabaseConnection con) {
		
		for(IDCPackage pack : packages) {
			pack.setParent(this);
			pack.setIsSystem(isSystem);
			pack.setDatabaseConnection(con);
	...
	public void init(IDCData userData) {
		...
		if(isInitRequired()) {
			...
			IDCUtils.debug("IDCApplication.init(): name = " + getName());
			...
			if(dbRefs.size() == 1) {
				...
				setDatabaseRef(ref);
				ref.init(userData);
				IDCUtils.debug("IDCApplication.init(): ref = " + ref.getURL());
    ...
    public void disconnect() throws Error {
		...
		if(!getName().equals(IDCAdminApplication.ADMIN_APPL)) {
    ...
    public long startTransaction() throws Error {
		...
		if(transactionId == NO_TRANSACTION) {
			...
			ret = transactionId;
			getDatabaseConnection().startTransaction();
		}
    ...
    public void endTransaction(long transactionId, boolean isCommit) throws Error {
		...
		if(this.transactionId == transactionId) {
			getDatabaseConnection().endTransaction(isCommit);
    ...
    public void createSchema(String schemaName) {
    	getDatabaseConnection().createSchema(schemaName);
    }
...
//		if(getConnectionDatabaseRef == null) {
