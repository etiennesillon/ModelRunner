...
public class IDCType extends IDCModelData {
	...
	public final static int ISTOPLEVELVIEWABLE=START_ATTR, ISTOPLEVELCREATABLE=START_ATTR+1, EXPLORERSQLFILTER=START_ATTR+2, EXPLORERFORMULAFILTER=START_ATTR+3,
							 EXPLORERSQLORDERBY=START_ATTR+4, PANELS=START_ATTR+5,  
							 DATABASEREF=START_ATTR+6, NAMEFORMULA=START_ATTR+7, ISUNIQUEKEY=START_ATTR+8,
							 DEFAULTLISTSTYLESHEET=START_ATTR+9, DEFAULTDETAILSSTYLESHEET=START_ATTR+10,
							 DEFAULTLISTPANEL=START_ATTR+11, DEFAULTDETAILSPANEL=START_ATTR+12, DEFAULTEDITDIALOG=START_ATTR+13, ISDATAENABLED=START_ATTR+14, EXTENDS_SYSTEM_TYPE=START_ATTR+15;
	...
	private String	  			  tableName, tableIdColName, explorerSQLFilter, explorerFormulaFilter, explorerSQLOrderBy, nameFormula, isDataEnabledFormula,
	...
	public void init(IDCData userData) {
		...
		if(isInitRequired()) {
			...
			explorerFormulaFilter = getString(EXPLORERFORMULAFILTER);
			...
			isDataEnabledStatus = getStatusFromFormula(isDataEnabledFormula);
    ...
    public int getStatusFromFormula(String formula) {
    ...
    public Object evaluate(String expr) {
    	...
    	IDCUtils.traceStart("IDCType.evaluate(expr=" + expr+")");
    	
    	Object ret = null;
    	IDCEvalData evalData = getEvalData(expr, null);
		if(evalData != null) {
			ret = evalData.getValue();
		}
    	
    	IDCUtils.traceEnd("IDCType.evaluate()");
    ...
    public IDCEvalData getEvalData(String exprStr, IDCData ref) {
    	...
    	IDCFormulaContext context = new IDCFormulaContext(null, ref, getApplication().getUser());
	...
	public String getExplorerFormulaFilter() {
    	return explorerFormulaFilter;
	}
...
//		String order = IDCFormula.getSQLFilter(getExplorerSQLOrderBy(), this);
	...
	public boolean isDataFiltered(IDCData data, String selectionFormula, IDCData refer) {

		IDCUtils.traceStart("isDataFiltered()");
		
		boolean ret = false;

		if(explorerFormulaFilter != null && explorerFormulaFilter.length() > 0) {
			if(data.isTrue(explorerFormulaFilter, refer)) {
				ret = true;
			}
		} else {
			ret = true;
		...
		IDCUtils.traceEnd("isDataFiltered()");
	...
	public List<IDCData> loadAllDataObjects(List<IDCDataRef> refs, String selectionFormula, IDCData refer, boolean getSuper) {
		...
		for(IDCDataRef ref : refs) {
			...
			if(isDataFiltered(data, selectionFormula, refer)) {
				ret.add(data);
			}
...
//		String where = IDCExpressionEvaluator.getSQLFilter(selectionFormula, this);
    ...
    public IDCData loadDataByName(String name, boolean isNeedCreate) {

		IDCData ret = requestSingleData("Name == '" + name + "'");
		
		if(ret == null && isNeedCreate) {
			ret = createData();
			ret.set("Name",  name);
			ret.save();
		}
	...
	public int requestCount(String formula) {
		return requestCount(formula, null);
	}

	public int requestCount(String formula, IDCDataParentRef parentRef) {
