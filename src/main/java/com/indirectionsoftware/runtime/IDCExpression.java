...
public class IDCExpression {
	...
	static final String[] VARIABLE_NAMES = {"Today", "Now", "Null", "This", "NewLine", "Ref", "ExtParent", "NSParent", "Id", "User", "Context", "NextId", "Name", "Link", "UserRole"};
	public static final int TODAY=0, NOW=1, NULL=2, THIS=3, NEWLINE=4, REF=5, EXTPARENT=6, NSPARENT=7, THISID=8, USER=9, CONTEXT=10, NEXTID=11, NAME=12, LINK=13, USER_ROLE=14;
	...
	public static List <IDCExpression> splitExpression(String expr) {
		...
		IDCUtils.traceStart("splitExpression(): expr = " + expr);
		...
		IDCUtils.traceStart("splitExpression(): ret = " + ret);
		...
		switch(sysEval.kwType) {
			...
			case VARIABLE:
				...
				switch(sysEval.kwIndex) {
					...
					case LINK:
						retType = IDCAttribute.STRING;
						retVal = IDCWebApplication.getLink(context.webAppContext, context.data.getDataType().getApplication(), context.data.getName(), context.data.getDataType().getId(), context.data.getId());
						break;
