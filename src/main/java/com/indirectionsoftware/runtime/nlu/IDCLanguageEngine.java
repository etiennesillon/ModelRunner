...
public class IDCLanguageEngine {
	...
	public IDCResults processSentence(String sentence, IDCWebAppContext context) {
		
		IDCResults ret = new IDCResults(context, sentence);
...
//					processWHQuery(ret);
	...
	public void processListQuery(IDCResults results) {
	...
	private void processYesNoQuery(IDCResults results) {

	}

	/*****************************************************************************/

	private void processWHQuery(IDCResults results) {

	}

	/*****************************************************************************/

	private IDCModelData getMostLikelyQueryType(IDCResults results, List<IDCWord> words) {
	...
	private void processListQueryConcept(IDCResults results, IDCConcept concept, List<IDCWord> preList, List<IDCWord> postList) {
...
//					}
	...
	public List<IDCWord> getWordsNEW(IDCResults results, String query) {
...
//					word.addValue(value);
	...
	public List<IDCAttributeValue> setSearch(IDCResults results, List<IDCAttributeValueDataList> attrVals) {
	...
	private List<IDCModelData> getEntities(IDCResults results, List<IDCWord> words, int entityType) {
...
//						}
	...
	private List<IDCModelData> getAttributes(IDCResults results, List<IDCWord> words) {
		return getEntities(results, words, IDCModelData.ATTRIBUTE);
	}

	/***************************************************************************************/

	private List<IDCModelData> getTypes(IDCResults results, List<IDCWord> words) {
	...
	private boolean processDataCommand(IDCResults results) {
	...
	private boolean  processNLUCommand(IDCResults results, String command) {
	   ...
	   for (int nWord = 0; nWord < words.length; nWord++) {
	      ...
	      } else if(nTo == -1 && word.startsWith("2")) {
		         nTo = nWord;
		         results.debugNLU("IDCLanguageEngine.processNLUCommand(): found 'set to' word = " + word);
		         setToValue = word.substring(1);
	      } else if(word.equals("called") || word.equals("call") || word.equals("cold") || word.equals("cold") || word.equals("gold")) {
	         nCalled = nWord;
