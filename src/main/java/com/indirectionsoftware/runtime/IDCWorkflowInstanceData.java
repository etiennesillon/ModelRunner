...
public class IDCWorkflowInstanceData extends IDCData {
	...
	public List<IDCError> execute() {
		...
		if(ret.size() == 0) {
			...
			IDCUtils.debug("xIDCWorkflowInstanceData.execute(): Executing Workflow: " + getName());
	...
	public List<IDCError> checkRequirements() {
		...
		for(IDCData req : getData(WORKFLOW_INSTANCE_WORKFLOW).getList(WORKFLOW_REQS)) {
			...
			if(contextData != null) {
				IDCUtils.debug("IDCWorkflowInstanceData.checkRequirements(): checkRequirementsfound required context data key " + key);
				...
				if(contextDataObject != null && contextDataObject.getDataType().getName().equals(typeName)) {
					IDCUtils.debug("found required context data" + contextDataObject);
					foundReq = true;
				}
			}
			...
			if(!foundReq) {
				IDCUtils.debug("IDCWorkflowInstanceData.checkRequirements(): Couldn't find required context data key=" + key);
	...
	public void addContextData(String key, IDCDataRef ref) {
		...
		workflowContext.save(false, false);
	...
	public static List<IDCWorkflowInstanceData> checkWorkflows(IDCData data) {
		...
		if(typeName.equalsIgnoreCase(WORKFLOW_TYPE) || typeName.equalsIgnoreCase(WORKFLOW_INSTANCE_TYPE)  || typeName.equalsIgnoreCase(IDCWorkflowInstanceStep.WORKFLOW_STEP_INSTANCE_TYPE)) {
			...
			IDCUtils.debug("IDCWorkflowInstanceData.checkWorkflows(): Checking workflow for type = " + typeName);
		...
		if(jsonWorkflow != null) {
			...
			if(wfName != null) {
				...
				try {
					...
					List<IDCError> errors = workflow.save(false, false); 
					if(errors.size() == 0) {
						if(oldWorkflow != null) {
