...
public class IDCWorkflowInstanceStep {
	...
	public void execute() {
		...
		if(!isCompleted()) {
			...
			IDCUtils.debug("IDCWorkflowInstanceStep.execute(): Preparing to execute Step = " + data.getName());
			...
			if(isReady) {
				...
				IDCUtils.debug("IDCWorkflowInstanceStep.execute(): Executing Step = " + data.getName());
				...
				for(IDCData  action : workflowStep.getList(WORKFLOW_STEP_ACTIONS)) {
					...
					IDCUtils.debug("IDCWorkflowInstanceStep.execute(): Executing objFormula = " + objFormula + " actionFormula = "+ actionFormula);
					...
					if(objFormula != null && objFormula.length() > 0) {
						...
						if(formulaDataObject != null) {
							...
							IDCUtils.error("IDCWorkflowInstanceStep.execute(): Could not evaluate Object Formula: " + objFormula);
						}
			...
			IDCUtils.debug("IDCWorkflowInstanceStep.execute(): Step = " + data.getName() + " already completed!");
		}
	...
	public boolean checkPredecessors(IDCData workflowStep) {
		...
		for(IDCData step : workflowStep.getList(WORKFLOW_STEP_PREDECESSORS)) {
			...
			IDCUtils.debug("IDCWorkflowInstanceStep.checkPredecessors(): Found Predecessor Type=" + step.getDisplayValue(WORKFLOW_STEP_PREDECESSOR_TYPE));
			...
			for(IDCData predStep : step.getList(WORKFLOW_STEP_PREDECESSOR_STEPS)) {
				IDCUtils.debug("IDCWorkflowInstanceStep.checkPredecessors(): Predecessor Step = " + predStep.getName());
				...
				if(predStepInstance != null && predStepInstance.isCompleted()) {
					IDCUtils.debug("IDCWorkflowInstanceStep.checkPredecessors(): Found Completed Predecessor Step Instance = " + predStepInstance.data.getName());
					foundPredecessorStepCompleted = true;
				} else {
					IDCUtils.debug("IDCWorkflowInstanceStep.checkPredecessors(): Found Not Completed Predecessor Step Instance = " + predStepInstance.data.getName());
	...
	public boolean checkRequirements(IDCData workflowStep) {
		...
		for(IDCData req : workflowStep.getList(WORKFLOW_STEP_REQS)) {
			...
			IDCUtils.debug("IDCWorkflowInstanceStep.checkRequirements(): Requirement Key = " + req.getName());
			...
			if(contextData != null) {
				...
				if(ref != null && ref.length() > 0) {
					IDCUtils.debug("IDCWorkflowInstanceStep.checkRequirements(): found required context data key " + key);
					...
					if(contextDataObject != null && contextDataObject.getDataType().getName().equals(typeName)) {
						IDCUtils.debug("IDCWorkflowInstanceStep.checkRequirements(): found required context data" + contextDataObject);
						if(formula != null && formula.length() > 0) {
							foundReq = contextDataObject.isTrue(formula);
							IDCUtils.debug("IDCWorkflowInstanceStep.checkRequirements(): formula = " + formula + " foundReq = " + foundReq);						
						} else {
							foundReq = true;
							IDCUtils.debug("IDCWorkflowInstanceStep.checkRequirements(): No formula foundReq = " + foundReq);						
						}
					}
				}
			}
			...
			if(!foundReq) {
				IDCUtils.debug("IDCWorkflowInstanceStep.checkRequirements(): Couldn't find required context data key=" + key);
				ret = false;
			}
		}
