...
import com.indirectionsoftware.runtime.nlu.IDCResults;
...
public class IDCWebApplication {
	...
	static final String BASE_URL = "WebApp";
					...
					if(action == IDCWebAppController.BACK && context.action == IDCWebAppController.TYPESEARCH || action == IDCWebAppController.POSTSEARCH) {
						...
						if(data != null) {
							if(data.isData()) {
								...
								ret = ((IDCResults)data).getFullHTML();
							}
						} else {
...
//							int nextPage=ITEMPAGE;
				...
				case IDCWebAppController.SEARCH:
					...
					if(term.length() > 0) {
						...
						setBrowser(context, search(context, term));
					} else {
						ret = getButtonsDivHTML(context);
						ret += "<div id=\"_datapane\" class=\"datapane\"><h1 class=\"pagetitle\">" + getPageTitle(context) + "</h1>";
						ret += "<p>Please enter something in the search box :)</p>";
					}
					break;
...
//					contexts.remove(contextId);
	...
	public String getSettingsHTML(IDCWebAppContext context) {
		...
		if(context.isUpdate) {
			...
			context.onKeyPress = " onKeyPress=\"if(event.key == 'Enter') {" + getClickUpdateFunction(context.contextId, IDCWebAppController.UPDATESETTINGS, IDCWebAppController.NA, IDCWebAppController.NA, true, "" + IDCWebAppController.NA) + "}\"";
		}
		...
		String onKeyPress = " onKeyPress=\"if(event.key == 'Enter') {" + IDCWebApplication.getClickUpdateFunction(context.contextId, IDCWebAppController.UPDATESETTINGS, IDCWebAppController.NA, IDCWebAppController.NA, true, "" + IDCWebAppController.NA) + "}\"";
	...
	public String getButtonsDivHTML(IDCWebAppContext context) {	
		...
		ret += "<li>" + getURLLink(context, "Help", IDCWebAppController.HELP, IDCWebAppController.NA, IDCWebAppController.NA, IDCWebAppController.NA, "", true, false) + "</li>";
	...
	private String getSearchButton(IDCWebAppContext context) {
		...
		String ret = "<li><form name='SearchForm'><input name='" + SEARCHFIELDNAME + "' placeholder='Search ...' value='" + context.searchVal + "' onKeyPress=\"if(event.key == 'Enter') {" + getClickUpdateFunction(context.contextId, IDCWebAppController.SEARCH, IDCWebAppController.NA, IDCWebAppController.NA, true, "" + IDCWebAppController.NA) + "}\"/></form></li>";
	...
	public String processSpeech(IDCWebAppContext context, String query) {
		...
		IDCResults results = app.getNLUEngine().processSentence(query, context);
...
//				even = !even;
	...
	public String getItemDetailsHTML(IDCWebAppContext context, Map<String, IDCError> errors) {
		...
		if(context.isUpdate) {
			ret += "<form name=\"UpdateItemDetails\">";
			context.onKeyPress = " onKeyPress=\"if(event.key == 'Enter') {" + getClickUpdateFunction(context.contextId, IDCWebAppController.UPDATEITEMSAVE, context.selectedType.getId(), context.selectedData.getId(), true, "" + IDCWebAppController.NA) + "}\"";
		}
...
//				IDCDataRef ref = (IDCDataRef) data.getRawValue(nAttr);
			...
			case IDCAttribute.EXTENSION:
				...
				rootHtml += "<select id=IDCField" + context.fieldId++ + " onchange=\"" + getClickUpdateFunction(context.contextId, IDCWebAppController.UPDATEITEMREFRESH, context.selectedType.getEntityType(), context.selectedData.getId(), true, "" + IDCWebAppController.NA) + "\" class='dropdown' name='" + getFieldName(data, attrId) + "'>";
...
//				rootHtml += "<script language='JavaScript'>new tcal ({'formname': 'updateform','controlname': '" + attr.getName() + "'});</script>";
	...
	String getURLLink(IDCWebAppContext context, String label, int action, long typeId, long itemId, long actionId, String attrIdStr, boolean isActive, boolean isCloseNav) {
		return "<a href=\"WebApp?" + IDCWebAppController.ACTION_PARM + "=" + action + "&" + IDCWebAppController.CONTEXTID_PARM + "=" + context.contextId + "&" + IDCWebAppController.TYPEID_PARM + "=" + typeId  + "&" + IDCWebAppController.ITEMID_PARM + "=" + itemId  + "&" + IDCWebAppController.ATTRID_PARM + "=" + attrIdStr + "&" + IDCWebAppController.ACTIONID_PARM + "=" + actionId+ "\">" + label + "</a>";
	}
	...
	String getURLButton(IDCWebAppContext context, String className, String label, int action, long typeId, long itemId, long actionId, String attrIdStr, boolean isActive, boolean isCloseNav, String imageName) throws Error {
		...
		String query = "'WebApp?" + IDCWebAppController.ACTION_PARM + "=" + action + "&" + IDCWebAppController.CONTEXTID_PARM + "=" + context.contextId + "&" + IDCWebAppController.TYPEID_PARM + "=" + typeId  + "&" + IDCWebAppController.ITEMID_PARM + "=" + itemId  + "&" + IDCWebAppController.ATTRID_PARM + "=" + attrIdStr + "&" + IDCWebAppController.ACTIONID_PARM + "=" + actionId+ "'";
		...
		if(action == IDCWebAppController.REMOVESELECTEDITEMS || action == IDCWebAppController.SELECTREFLISTOK) {
			...
			func += "speak(" + query + ");";
		} else if(action == IDCWebAppController.HELP) {
			func += "help();";
			...
			if(label == null) {
				func += getClickUpdateFunction(context.contextId, action, typeId, itemId, isActive, "" + IDCWebAppController.NA);
				label = "Update";
				type = "type = \"submit\"";
			} else {
//				func += "reloadPost('IDCWebAppController'," + action + "," + typeId + "," + itemId + ",'" + attrIdStr + "'); return false;"; 
				func += getClickUpdateFunction(context.contextId, action, typeId, itemId, isActive, "'" + attrIdStr + "'");
			}
		} else {
	...
	public static String getLink(IDCWebAppContext context, IDCApplication app, String label, long typeId, long itemId) throws Error {
		return getLink(context, app, label, IDCWebAppController.GETITEMDETAILS, typeId, itemId);
	}

    /****************************************************************************/
    
	public static String getLink(IDCWebAppContext context, IDCApplication app, String label, int action, long typeId, long itemId) throws Error {
		String query = "'IDCWebApp?" + IDCWebAppController.ACTION_PARM + "=" + action + "&" + IDCWebAppController.CONTEXTID_PARM + "=" + context.contextId + "&" + IDCWebAppController.TYPEID_PARM + "=" + typeId  + "&" + IDCWebAppController.ITEMID_PARM + "=" + itemId + "'";
	...
	static String getClickUpdateFunction(String appId, int action, long typeId, long itemId, boolean isActive, String attrIdStr) throws Error {
		return "event.preventDefault(); reloadPost('WebApp', '" + appId + "', " + action + "," + typeId + "," + itemId + "," + attrIdStr + "); return false;";
	}
	...
	public String getPageTitle(IDCWebAppContext context) {
		...
		switch(context.action) {
			...
			case IDCWebAppController.TYPESEARCH:
			case IDCWebAppController.SEARCH:
				ret = "Search " + (context.selectedType == null ? "" : IDCUtils.getPlural(context.selectedType.getDisplayName()));
				break;
...
//  		ret = getUpdateHTML(context, context.selectedData, nAttr, attr, false, true);
    ...
    public List<IDCDataRef> search(IDCWebAppContext context, String searchTerm) {
    	
    	List<IDCDataRef> ret = new ArrayList<IDCDataRef>();
    	
    	for(IDCType type : context.webApp.getApplication().getTypes()) {
	    	...
	    	for(IDCDataRef ref : type.loadAllDataReferences()) {
	    		...
	    		for(IDCAttribute attr : type.getAttributes()) {
        			...
        			if(val != null) {
        				...
        				IDCUtils.debug("IDCWebPageContext.searchFromHTML(): searchVal = " + searchTerm + " / val = " + val);

        				switch(attr.getAttributeType()) {
        				
        					case IDCAttribute.DOMAIN:
        						if(searchTerm == ((IDCDomainValue)val).getName()) {
        							isMatchFound = true;
        						}
        						break;
        					...
        					case IDCAttribute.REF:
        					case IDCAttribute.REFTREE:
        						if(searchTerm.equals(((IDCData)val).getName())) {
        							isMatchFound = true;
        						}
        						break;
        					...
        					default:
        					    ...
        					    if(isStartsWith && isEndsWith) {
            						if(valStr.indexOf(searchTerm) != -1) {
            							isMatchFound=true;
            						}
            						
        					    } else if(isStartsWith) {
            						if(valStr.startsWith(searchTerm)) {
            							isMatchFound=true;
            						}
        					    } else if(isEndsWith) {
            						if(valStr.endsWith(searchTerm)) {
            							isMatchFound=true;
            						}
        					    } else {
            						if(valStr.equalsIgnoreCase(searchTerm)) {
            							isMatchFound=true;
            						}
        					    }
        						break;
    	...
    	for(IDCType type : context.webApp.getApplication().getTypes()) {
	    	...
	    	for(IDCDataRef ref : type.loadAllDataReferences()) {
	    		...
	    		for(IDCAttribute attr : type.getAttributes()) {
	    			...
	    			Object searchVal = data.getRawValue(nAttr);
	    			Object searchValObj = data.getValue(nAttr);
...
//				context.resetBrowserMap();
	...
	public void setContext(HttpServletRequest request, IDCWebAppContext context) {
		request.getSession().setAttribute(IDCWebAppController.SESSIONID + context.contextId, context);
		IDCWebAppController.debugSession(request);
	}
