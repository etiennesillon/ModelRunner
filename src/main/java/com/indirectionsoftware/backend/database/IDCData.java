...
public class IDCData extends IDCVector implements IDCEnabled {
   ...
   /************************************************************************************************/
    ...
    public List<IDCError> save(boolean isCheckWorkflows, boolean isCheckData) {

    	List<IDCError> ret = new ArrayList<IDCError>();
    	
    	if(isCheckData) {
    		ret.addAll(checkData());
    	}
...
//        	refMap.put(new IDCDataRef(this).toString(),"");
   ...
   public String getJSONString(boolean isExpanded, boolean isFirstChild) {
	...
	public boolean isSystemApp() {
		...
		if(type.getName().equals(IDCAdminApplication.APPLICATION_TYPE) && getName().equals(IDCAdminApplication.ADMIN_APPL)) {
			ret = true;
		}
