    ...
    public class IDCRefTree {
		...
		class IDCSelectBox implements ItemListener {
			...
			public String getHTML(IDCWebAppContext context, String labelName, String prefix, String fieldName, int nBox, boolean isFilled, boolean isLast) {
				...
				String ret = "<select id=IDCField" + context.fieldId++ + " onChange=\"return updateRefTree('IDCWebAppController?" + IDCWebAppController.ACTION_PARM + "=" + IDCWebAppController.RELOADREFTREE  + "&" + IDCWebAppController.APPID_PARM + "=" + app.getName() + "', '" + labelName + "', '" + prefix + "', " + nBox + ", '" + fieldName + "');\" class='dropdown' name='" + fieldName + "'>";
