...
var xAxisUnitNum = 11, yAxisUnitNum = 10;

var borderWidth = 50;
var xAxis0 = 50, yAxis0 = 550;
var xAxisLength = 1000, yAxisLength=500
...
const BACKGROUND_COLOUR = "#ffffff"; //"#eaf4fa";
...
/*********************************************************************/
  ...
  function initData(graphData) {
	...
	for(var nPoint=0; nPoint < graphData.Points.length; nPoint++) {

		var dataPoint = new Data(graphData.Points[nPoint])

		var x = dataPoint.values[xData];
		if(xMax == null || x > xMax) {
			xMax = x;
		}
		if(xMin == null || x < xMin) {
...
function repaint() {
	...
	drawRoundedRectangle(0, 0, canvas.width, canvas.height, 5, false, true);
...
function drawAxis() {
	...
	for(var nUnit=0; nUnit <= xAxisUnitNum; nUnit++) {
		...
		ctx.fillText(getDateString(xVal), x, yAxis0 + (on ? 15 : 25));   
		x+= xAxisUnit;
		xVal += xAxisUnitIncr;
		on = !on;
	}
	...
	for(var nUnit=0; nUnit <= yAxisUnitNum; nUnit++) {
		...
		var label = getAmountString(yVal);
		var l = ctx.measureText(label).width;
		console.log("l = " + l);
		ctx.fillText(label, xAxis0 - 10 - l, y);   
		y-= yAxisUnit;
		yVal += yAxisUnitIncr;
	}
...
function drawPoints() {
	...
	for(var nPoint=0; nPoint < data.length; nPoint++) {
		var x = xAxis0 + ((data[nPoint].values[xData] - xMin) / xAxisRatio);
		var y = yAxis0 - ((data[nPoint].values[yData] - yMin) / yAxisRatio);
		if(prevX != null) {
...
function getAmountString(amount) {
	...
	return ret + dollars + "." + (cents < 10 ? "0" : "" ) + cents; 
}
...
function resizeCanvas() {
   ...
   canvasTop = 0; 
	...
	xAxisLength = canvasWidth - borderWidth * 2;
	yAxisLength = canvasHeight - borderWidth * 2;

	xAxisRatio = (xMax - xMin) / xAxisLength;
	yAxisRatio = (yMax - yMin) / yAxisLength;
	...
	xAxis0 = borderWidth;
	yAxis0 = canvasHeight - borderWidth;
